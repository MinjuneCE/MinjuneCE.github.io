...
const CASE_TRACE = [
  {
    n: '00', t: '발단 — 떠안은 난제',
    body: ['난이도가 높아 원래 **PL 담당**으로 분장됐던 다중화 task. PL이 프로젝트 중반 중도 하차하자 PM이 나에게 재할당했고, 다중화를 직접 학습해 단독으로 해결에 들어갔다.'],
  },
  {
    n: '01', t: '제약 — 계획이 무너지다',
    body: [
      '사전 인터뷰 답변대로 **L4 스위치 전제**로 제안서·계획을 작성했으나, 착수 후 L4 도입 불가가 판명됐다.',
      '게다가 파일을 받는 **다중화 송수신 솔루션이 단일 서버에 종속**돼 있고 유지보수 인력도 없어 복제가 불가능했다. → 8개 인프라 중 1대를 **단일 수신 게이트웨이(처리도 겸함)**로 두고 나머지로 처리를 다중화하는 구조가 강제됐다.',
    ],
  },
  {
    n: '02', t: '1차 분기 — 공유 디렉토리 폴링을 버리다',
    body: [
      '모든 서버가 공유 디렉토리를 폴링하며 파일을 집어가는 안을 검토했으나 기각. ① 수신이 1서버 종속이라 대칭 수신+처리 자체가 불가, ② **파일시스템엔 원자적 claim 수단이 없어** 1천만 건에서 여러 워커가 같은 파일을 동시에 집는 것을 막기 어렵다.',
      '메타데이터가 **DB에 있으니 트랜잭션 락으로 원자적 claim**이 가능했다 — 어쩔 수 없이가 아니라, 더 우월한 claim 수단을 보고 **능동적으로 DB 작업 큐를 선택**했다.',
    ],
  },
  {
    n: '03', t: '2차 분기 — 분배(push)에서 선점(pull)으로',
    body: [
      '처음엔 master가 작업을 나눠주는 구상을 했다. 그러나 단순 시퀀셜(round-robin) 분배는 **파일 크기 편차로 일부 서버만 혹사**되는 부하 불균형(makespan 악화)이 문제였다 — 가용성이 아니라 부하분산 문제다.',
      '여기에 고객이 **인수:품질 서버 비율을 3:5 / 2:6 등으로 유동 운영**하고 싶다는 요구를 더했다. → 분배자를 두지 않고 **워커가 직접 선점하는 자율 할당(경쟁 소비자 / work-stealing)**으로 전환했다.',
    ],
  },
  {
    n: '04', t: '동시성 함정 — 데드락, 그리고 SKIP LOCKED',
    body: [
      '대상을 **10건씩 묶어 claim**하자 워커들이 경합하며 **데드락**이 발생했다. 여러 행을 한 트랜잭션에서 잠글 때 **일관된 `ORDER BY`가 없으면 락 획득 순서가 비결정적**이 되어 순환 대기에 빠진 것.',
      '**배치=1**로 줄이자 단일 락이라 순환 대기가 불가능해 데드락은 사라졌지만, DB 왕복이 급증해 **처리량이 떨어졌다**.',
      '최종 해법은 **`SELECT … FOR UPDATE SKIP LOCKED`** — 잠긴 행을 건너뛰므로 배치를 다시 키워도 락 대기·순환 대기 없이 **처리량을 회복**했다.',
    ],
  },
  {
    n: '05', t: '리소스 함정 — 커넥션 풀 사이징',
    body: [
      '8서버 × 멀티스레드 워커의 동시 사용 주체가 풀 크기를 넘어 **커넥션 획득 실패**가 났다. claim 트랜잭션은 이미 짧아 점유 시간이 아니라 **순수 동시성 사이징** 문제였다.',
      '인프라팀과 조율해 워커 동시성에 맞춰 풀을 증설했다 — 이 경우의 증설은 임시방편(band-aid)이 아니라 **정당한 리소스 사이징**이다.',
    ],
  },
  {
    n: '06', t: '실패 모델 — soft / hard 분리',
    body: [
      '**Soft failure**(IO·변환·비즈니스 예외): `try-catch`로 상태를 `오류`로 마킹 + 로그 → 유지보수 조직 확인. 자동으로 동작한다.',
      '**Hard crash**(`kill -9`·OOM·전원·네트워크 단절): catch가 실행되지 않아 task가 `작업중`에 stuck된다. claim 시 **`worker_id`(서버 식별자)를 행에 기록**해 두므로, 실시간 모니터링이 죽은 서버를 감지하면 그 식별자의 stuck 행을 **역추적해 수동 회수**한다.',
      '**리퍼(reaper)·lease 타임아웃은 의도적으로 미도입** — 무중단 자동복구가 불요한 업무다. 그래서 **멱등성도 현재는 비필수**다. (자동 회수를 넣으면 "완료 후 마킹 전 크래시 → 이중 처리" 위험이 생겨, 그때는 처리-마킹 원자화나 멱등 처리가 필수가 된다.)',
    ],
  },
  {
    n: '07', t: '닫음 — 의도적으로 낮춘 복잡도',
    body: [
      '수신 SPOF, 역할 pin, 설정 변경 시 재기동, 수동 회수 — 이들은 약점이 아니라 **요구와 제약을 읽고 복잡도를 의도적으로 낮춘 결정**이다. 무중단 자동복구 불요 → 수동 회수로 충분 → 멱등성 불요 → **단순성 확보**로 이어지는 일관된 사슬.',
    ],
  },
];

const CASE_QA = [
  { q: '같은 작업을 두 워커가 동시에 잡지 않나?', a: '`SELECT … FOR UPDATE SKIP LOCKED`로 행을 잠그고 **즉시 `작업중`으로 바꿔 커밋**하는 원자적 claim이다. 이미 잠긴 행은 다른 워커가 **건너뛰므로** 중복 선점이 구조적으로 불가능하다.' },
  { q: '부하는 어떻게 고르게 분산되나?', a: '분배자가 미리 나눠주는 push가 아니라 **워커가 끝나는 대로 다음 작업을 가져가는 pull(work-stealing)**이라, 파일 크기 편차가 있어도 빠른 워커가 더 처리해 **자연스럽게 부하가 평탄화**된다.' },
  { q: '워커가 처리 도중 죽으면 그 작업은?', a: 'hard crash는 `catch`가 돌지 않아 행이 `작업중`에 고착된다. claim 시 기록한 **`worker_id`로 죽은 서버의 stuck 행을 역추적**해, 모니터링이 서버 다운을 감지한 뒤 **수동 회수**한다. 자동 리퍼는 무중단 복구가 불요해 두지 않았다.' },
  { q: 'DB·수신이 단일인데 완전한 HA인가?', a: '아니다. **처리 계층은 8서버 HA·부하분산**이지만 수신 게이트웨이는 솔루션 제약상 **단일(SPOF)**이고 작업 큐 DB도 공유 자원이다. 숨기지 않는다 — 주어진 제약 안에서 **처리 계층의 가용성과 확장성**을 확보한 것이 이 설계의 범위다.' },
  { q: '배치를 줄였다 다시 키운 이유는?', a: '배치 10 → **데드락**(락 순서 비결정) → 배치 1 → 데드락은 풀렸으나 **DB 왕복↑로 처리량↓** → **`SKIP LOCKED`로 배치 복원**해 처리량까지 회복. 줄인 게 해법이 아니라 SKIP LOCKED가 최종 해법이다.' },
  { q: '멱등성은 보장하나?', a: '자동 재처리가 없어 **현재는 비필수**다. 자동 회수를 도입하면 "완료 후 마킹 전 크래시 → 이중 처리" 위험이 생기므로, 그때는 **처리-마킹 원자화 또는 멱등 처리**가 필수로 전환된다.' },
];
      ...
      <div className="container">
        ...
        <div className="case-intro reveal">
          ...
          <p className="case-lead">
            행정안전부 중앙영구기록관리시스템 다중화(2024). 약 <strong>1천만 단위</strong> 파일 처리를 분배자 없는 <strong>경쟁 소비자(self-claiming)</strong> 구조로 풀어낸 기록 — 제약이 설계를 강제하고, 한 해결이 다음 문제를 부른 추적기.
          </p>
        ...
        <div className="case-section-label"><span>01 — 설계 추적 (Design Trace)</span></div>
        ...
        <div className="case-section-label"><span>04 — 설계 노트 · 실패 모드 (면접 떡밥)</span></div>
